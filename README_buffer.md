...
기존 테스트는 B-Tree 로직이 정확히 구현되었는지를 확인하기 위해, 모든 레코드와 엔트리가 올바른 값과 위치에 매칭되어 있는지를 전부 검사하는 방식이었습니다. 이 때문에 제가 직접 트리 구조를 그림으로 그려가며 하나하나 확인해야 했고, 자연스럽게 테스트할 수 있는 데이터 규모도 매우 작을 수밖에 없었습니다.

테스트 규모가 작았다는 점이 아쉬워 “어떻게 하면 더 큰 데이터셋으로 테스트할 수 있을까?”를 고민했고, 그 결과 테스트 전략 자체를 변경했습니다.

새로운 방식은 트리의 전체 구조를 완전히 검증하는 대신, 작업 수행 결과에서 실제 데이터의 key 값을 정상적으로 조회할 수 있는지를 확인하는 방식으로 테스트합니다. 이 방식은 완전한 트리 구조 검증에는 한계가 있지만, 훨씬 더 큰 데이터셋을 사용해 테스트할 수 있다는 장점이 있습니다. 이를 통해 이전 프로젝트에서는 발견하지 못했던, 대규모 데이터 상황에서만 드러나는 문제들을 확인할 수 있었습니다.
